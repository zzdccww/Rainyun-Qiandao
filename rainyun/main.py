                            ...
                            if check_answer(result):
                                for position in result.positions:
                                    ...
                                    try:
                                        ...
                                        logger.warning(f"{prefix}JS 点击失败，尝试 ActionChains: {click_error}")
                                    if not clicked:
...
def download_captcha_assets(ctx: RuntimeContext) -> tuple[bytes, np.ndarray, list[np.ndarray]]:
    ...
    captcha_bytes = b""
    try:
        captcha_bytes = slide_bg.screenshot_as_png
        if not captcha_bytes:
            raise ValueError("captcha screenshot empty")
        logger.info(f"{prefix}使用元素截图获取验证码背景图")
        ...
        logger.warning(f"{prefix}验证码背景截图失败，回退下载: {screenshot_error}")
    ...
    sprite_bytes = b""
    try:
        sprite_bytes = sprite.screenshot_as_png
        if not sprite_bytes:
            raise ValueError("sprite screenshot empty")
        logger.info(f"{prefix}使用元素截图获取验证码小图")
    except Exception as screenshot_error:
        img2_url = sprite.get_attribute("src")
        logger.info(f"{prefix}开始下载验证码图片(2): {img2_url}")
        logger.warning(f"{prefix}验证码小图截图失败，回退下载: {screenshot_error}")
